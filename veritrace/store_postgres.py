...
Usage
-----
    from veritrace.store_postgres import PostgresStore
    db = PostgresStore.from_dsn(
        "postgresql://user:pass@host:5432/veritrace",
        max_pool_size=10,
        max_retries=3,
    )
    armor = Veritrace(store=db, audit=db)
"""
...
class _PostgresBase:
    ...
    _DDL_TRACES = """
    CREATE TABLE IF NOT EXISTS vt_traces (
    ...
    CREATE INDEX IF NOT EXISTS vt_traces_tenant ON vt_traces(tenant_id);
    CREATE INDEX IF NOT EXISTS vt_traces_created ON vt_traces(created_at);
    """

    _DDL_CHAIN = """
    CREATE TABLE IF NOT EXISTS vt_chain (
        id          BIGSERIAL PRIMARY KEY,
    ...
    def _execute_ddl(self) -> None:
        ...
        with conn.cursor() as cur:
            ...
            cur.execute("SELECT this_hash FROM vt_chain ORDER BY id DESC LIMIT 1")
            row = cur.fetchone()
            if row:
                self._head = row[0]
        conn.commit()
...
class PostgresStore(_PostgresBase):
    ...
    ``store=db`` and ``audit=db`` to Veritrace.
    """
    ...
    def save(self, trace) -> None:
        ...
        def _fn(conn, cur):
            cur.execute(
                """
                INSERT INTO vt_traces (tenant_id, session_id, trace_id, payload)
                VALUES (%s, %s, %s, %s)
    ...
    def get(self, trace_id: str) -> Optional[dict]:
        def _fn(conn, cur):
            cur.execute("SELECT payload FROM vt_traces WHERE trace_id = %s", (trace_id,))
            row = cur.fetchone()
    ...
    def list_for_tenant(self, tenant_id: str, limit: int = 100) -> List[dict]:
        def _fn(conn, cur):
            cur.execute(
                "SELECT payload FROM vt_traces WHERE tenant_id = %s ORDER BY created_at DESC LIMIT %s",
                (tenant_id, limit),
            )
    ...
    def delete_for_tenant(self, tenant_id: str) -> int:
        def _fn(conn, cur):
            cur.execute("DELETE FROM vt_traces WHERE tenant_id = %s", (tenant_id,))
            return cur.rowcount
        return self._run(_fn)
    ...
    def prune_older_than(self, cutoff_ts: float) -> int:
        ...
        def _fn(conn, cur):
            cur.execute("DELETE FROM vt_traces WHERE created_at < %s", (dt,))
            return cur.rowcount
        return self._run(_fn)
    ...
    def append(self, payload: dict, prev_hash: str) -> tuple[str, str]:
        ...
        def _fn(conn, cur):
            cur.execute(
                """
                INSERT INTO vt_chain (this_hash, prev_hash, payload)
                VALUES (%s, %s, %s)
    ...
    def verify(self) -> list[dict]:
        ...
        def _fn(conn, cur):
            cur.execute(
                "SELECT this_hash, prev_hash, payload FROM vt_chain ORDER BY id ASC"
            )
            return cur.fetchall()
        rows = self._run(_fn)
