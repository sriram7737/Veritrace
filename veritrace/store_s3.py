    ...
    def _load_s3_client() -> Any:
        try:
            ...
            raise S3ArchiveError(
                "boto3 is not installed; install with: pip install 'veritrace[s3]'"
            ) from exc
        return boto3.client("s3")

    @staticmethod
    def _load_fernet(encryption_key: bytes | str | None) -> Any:
        key = encryption_key or os.environ.get("VERITRACE_ARCHIVE_KEY")
        if not key:
            raise ValueError(
                "S3 archive encryption key required: pass encryption_key= "
                "or set VERITRACE_ARCHIVE_KEY"
            )
        try:
            ...
            raise S3ArchiveError(
                ...
                "install with: pip install 'veritrace[s3]'"
            ) from exc
