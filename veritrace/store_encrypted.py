...
VERITRACE_ENCRYPTION_KEY environment variable. Generate one with::
...
class EncryptedSQLiteStore:
    ...
    def __init__(self, path: str = "veritrace.db", key: bytes | str | None = None) -> None:
        try:
            from cryptography.fernet import Fernet
        except ImportError as e:
            raise RuntimeError(
                "EncryptedSQLiteStore requires the 'cryptography' package. "
                "Install with: pip install 'veritrace[encrypted]'"
            ) from e

        if key is None:
            key = os.environ.get("VERITRACE_ENCRYPTION_KEY")
        if not key:
            raise ValueError(
                "encryption key required: pass key= or set VERITRACE_ENCRYPTION_KEY"
            )
